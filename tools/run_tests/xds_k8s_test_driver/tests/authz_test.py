...
class AuthzTest(xds_k8s_testcase.SecurityXdsKubernetesTestCase):
    ...
    def test_plaintext_allow(self) -> None:
        ...
        with self.subTest('03_header_regex'):
            ...
            self.configure_and_assert(
                test_client, 'header-regex-ab', grpc.StatusCode.PERMISSION_DENIED)
            self.configure_and_assert(
                test_client, 'aheader-regex-a', grpc.StatusCode.PERMISSION_DENIED)
        ...
        with self.subTest('05_never_match_host'):
            self.configure_and_assert(
                test_client, 'never-match-host', grpc.StatusCode.PERMISSION_DENIED)

        with self.subTest('06_never_match_port'):
            self.configure_and_assert(
                test_client, 'never-match-port', grpc.StatusCode.PERMISSION_DENIED)

        with self.subTest('07_principal_present'):
            self.configure_and_assert(
                test_client, 'principal-present', grpc.StatusCode.PERMISSION_DENIED)
    ...
    def test_mtls_allow(self) -> None:
        ...
        with self.subTest('05_never_match_principal'):
            self.configure_and_assert(
                test_client, 'never-match-principal', grpc.StatusCode.PERMISSION_DENIED)
    ...
    def test_plaintext_deny(self) -> None:
        ...
        with self.subTest('02_no_match'):
            self.configure_and_assert(
                test_client, None, grpc.StatusCode.OK)
